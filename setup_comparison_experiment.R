...
#usage Rscript seup_experiment.R <name of experiment> <max runs> <chunk size>
...
# now make a new version of run_matrix where all non-refieldian processes are swiched off

redfieldian_run_matrix <- run_matrix
redfieldian_run_matrix$PONdeg_factor<-redfieldian_run_matrix$redfield
redfieldian_run_matrix$nitrate_to_slDON_conv<-0
redfieldian_run_matrix$NH4_turnover<-0
redfieldian_run_matrix$TEP_fraction<-0
redfieldian_run_matrix$slDOM_C_TO_N<-redfieldian_run_matrix$redfield
redfieldian_run_matrix$slDON_deg<-1
redfieldian_run_matrix$slDOC_deg_factor<-1
redfieldian_run_matrix$TEPdeg<-1

#now make a physics only version
physics_run_matrix<-redfieldian_run_matrix
physics_run_matrix$WINTER_NITRATE<-0
physics_run_matrix$redfield<-6.6
physics_run_matrix$POCdeg<-1e6
physics_run_matrix$Q10_REF_TEMP<-15
physics_run_matrix$Q10<-2
physics_run_matrix$MIN_BOTTOM_TEMP<-5
physics_run_matrix$MAX_BOTTOM_TEMP<-10


run_matrix<-rbind(run_matrix,redfieldian_run_matrix,physics_run_matrix)
