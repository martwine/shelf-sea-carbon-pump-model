...
eval_DIC<-function(DIC,dNO3,pCO2,pCO2_atmos,temp,slDOC){
	DIC-calc_DIC_uptake_from_NO3(dNO3)+calc_as_flux(pCO2,pCO2_atmos,temp)+calc_slDOC_deg(slDOC)
}
...
model_run<-function(run_name){
	...
	timestep = 1
	model_output<-data.frame(pCO2=winter_pCO2_atmos,DIC=init_DIC,slDON=0,slDOC=0)
